...
// The pod condition helpers below are a fork of kubernetes/pkg/api/v1/pod/utils.go. They live here
// rather than in each controller because several of them need the same answer to "is this pod ready".
...
// GetPodReadyCondition extracts the pod ready condition from the given status, or nil if it is not
// present.
...
// GetPodCondition extracts the provided condition from the given status and returns its index and the
// condition. Returns -1 and nil if the condition is not present.
